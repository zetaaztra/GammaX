...
graph TD
    subgraph "Data Acquisition Layer"
        YF[Yahoo Finance API] -->|Fetch OHLCV| DM[scripts/data_manager.py]
        DM -->|Save/Update| CSV[(Raw CSV Cache<br/>public/data/raw/*.csv)]
        CSV -->|Read History| PL[scripts/tradyxa_pipeline.py]
    end
    ...
    subgraph "Processing & Simulation Layer"
        ...
        FE --> JSON_INIT[Initial Ticker JSONs<br/>public/data/ticker/*.json]
        SIM --> JSON_INIT
    end
...
### 1. Data Fetching & Storage (`scripts/data_manager.py`)
...
### 2. Pipeline Processing (`scripts/tradyxa_pipeline.py`)
...
*   **Output**: Generates "Initial" JSON files in `public/data/ticker/{ticker}.json`. These contain the computed metrics and features but *not yet* the ML predictions.
...
### 4. Model Application (`scripts/apply_models.py`)
